...
#define axisForEdge(d) ((GTAxis)isEdgeVertical(d))
#define perpAxis(d) ((GTAxis)(!(BOOL)d))

typedef NS_ENUM(NSInteger, GTAxis) {
    GTAxisX = 0,
    GTAxisY = 1
};
...
static inline GTPoint GTPointMake(double x, double y)
{
    GTPoint p; p.x = x; p.y = y; return p;
}

typedef union GTTrapezoid {
    struct { GTPoint a, b, c, d; };
    GTPoint v[4];
} GTTrapezoid;


typedef struct GTSegment {
    GTPoint a;
    GTPoint b;
} GTSegment;

static inline GTSegment GTSegmentMake(GTPoint a, GTPoint b)
{
    GTSegment s; s.a = a; s.b = b; return s;
}

typedef GTSegment GTBezierCurve;

static const int GTTrapezoidWinding[4][4] = {
    [GTRectEdgeTop]    = {0,1,2,3},
    [GTRectEdgeLeft]   = {2,0,3,1},
    [GTRectEdgeBottom] = {3,2,1,0},
    [GTRectEdgeRight]  = {1,3,0,2},
};
...
- (void)genieInTransitionWithDuration:(NSTimeInterval)duration destinationRect:(CGRect)destRect destinationEdge:(GTRectEdge)destEdge completion:(void (^)(void))completion {
...
- (void)genieOutTransitionWithDuration:(NSTimeInterval)duration startRect:(CGRect)startRect startEdge:(GTRectEdge)startEdge completion:(void (^)(void))completion {
    [self genieTransitionWithDuration:duration
                                 edge:startEdge
                          ...
                          completion:(void (^)(void))completion
{
    ...
    GTAxis axis = axisForEdge(edge);
    GTAxis pAxis = perpAxis(axis);
    ...
    // Bezier calculations
    CGFloat xInset = axis == GTAxisY ? -kRenderMargin : 0.0f;
    CGFloat yInset = axis == GTAxisX ? -kRenderMargin : 0.0f;
    ...
    GTSegment aPoints = bezierEndPointsForTransition(edge, [self convertRect:CGRectInset(self.bounds, xInset, yInset) toView:self.superview]);
    
    GTSegment bEndPoints = bezierEndPointsForTransition(edge, marginedDestRect);
    GTSegment bStartPoints = aPoints;
    bStartPoints.a.v[axis] = bEndPoints.a.v[axis];
    bStartPoints.b.v[axis] = bEndPoints.b.v[axis];
    
    GTBezierCurve first = {aPoints.a, bStartPoints.a};
    GTBezierCurve second = {aPoints.b, bStartPoints.b};
...
- (UIImage *) renderSnapshotWithMarginForAxis:(GTAxis)axis
{
...
- (NSArray *) sliceImage: (UIImage *) image toLayersAlongAxis: (GTAxis) axis
{
    CGFloat totalSize = axis == GTAxisY ? image.size.height : image.size.width;
    
    GTPoint origin = {0.0, 0.0};
    origin.v[axis] = kSliceSize;
    
    CGFloat scale = image.scale;
    CGSize sliceSize = axis == GTAxisY ? CGSizeMake(image.size.width, kSliceSize) : CGSizeMake(kSliceSize, image.size.height);
...
- (NSArray *) transformationsForSlices: (NSArray *) slices
                         ...
                         startPosition: (CGFloat) startPosition
                           ...
                           firstBezier: (GTBezierCurve) first
                          secondBezier: (GTBezierCurve) second
                        finalRectDepth: (CGFloat) rectDepth
{
    ...
    __block GTTrapezoid trapezoid = {0};
    trapezoid.v[GTTrapezoidWinding[edge][0]] = bezierAxisIntersection(first, axis, position);
    trapezoid.v[GTTrapezoidWinding[edge][1]] = bezierAxisIntersection(second, axis, position);
    ...
    [slices enumerateObjectsWithOptions:enumerationOptions usingBlock:^(CALayer *layer, NSUInteger idx, BOOL *stop) {
        ...
        if (overflow <= 0.0f) { // slice is still in bezier part
            trapezoid.v[GTTrapezoidWinding[edge][2]] = bezierAxisIntersection(first, axis, endPosition);
            trapezoid.v[GTTrapezoidWinding[edge][3]] = bezierAxisIntersection(second, axis, endPosition);
        }
            ...
            trapezoid.v[GTTrapezoidWinding[edge][2]] = first.b;
            trapezoid.v[GTTrapezoidWinding[edge][2]].v[axis] += sign*shrunkSliceDepth;
            
            trapezoid.v[GTTrapezoidWinding[edge][3]] = second.b;
            trapezoid.v[GTTrapezoidWinding[edge][3]].v[axis] += sign*shrunkSliceDepth;
        }
        ...
        trapezoid.v[GTTrapezoidWinding[edge][0]] = trapezoid.v[GTTrapezoidWinding[edge][2]]; // next one starts where previous one ends
        trapezoid.v[GTTrapezoidWinding[edge][1]] = trapezoid.v[GTTrapezoidWinding[edge][3]];
...
- (CATransform3D) transformRect: (CGRect) rect toTrapezoid: (GTTrapezoid) trapezoid
{
...
static GTSegment bezierEndPointsForTransition(GTRectEdge edge, CGRect endRect)
{
    switch (edge) {
        case GTRectEdgeTop:
            return GTSegmentMake(GTPointMake(CGRectGetMinX(endRect), CGRectGetMinY(endRect)), GTPointMake(CGRectGetMaxX(endRect), CGRectGetMinY(endRect)));
        case GTRectEdgeBottom:
            return GTSegmentMake(GTPointMake(CGRectGetMaxX(endRect), CGRectGetMaxY(endRect)), GTPointMake(CGRectGetMinX(endRect), CGRectGetMaxY(endRect)));
        case GTRectEdgeRight:
            return GTSegmentMake(GTPointMake(CGRectGetMaxX(endRect), CGRectGetMinY(endRect)), GTPointMake(CGRectGetMaxX(endRect), CGRectGetMaxY(endRect)));
        case GTRectEdgeLeft:
            return GTSegmentMake(GTPointMake(CGRectGetMinX(endRect), CGRectGetMaxY(endRect)), GTPointMake(CGRectGetMinX(endRect), CGRectGetMinY(endRect)));
    }
...
static GTPoint bezierAxisIntersection(GTBezierCurve curve, GTAxis axis, CGFloat axisPos)
{
...
static inline NSString * edgeDescription(GTRectEdge edge)
{
    NSString *rectEdge[] = {
        [GTRectEdgeBottom] = @"bottom",
        [GTRectEdgeTop] = @"top",
        [GTRectEdgeRight] = @"right",
        [GTRectEdgeLeft] = @"left",
    };
