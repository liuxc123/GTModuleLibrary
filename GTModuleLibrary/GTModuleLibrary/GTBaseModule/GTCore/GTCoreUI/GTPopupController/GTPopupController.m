#import "GTPopupController.h"
#import <objc/runtime.h>

@interface GTPopupController () <UIGestureRecognizerDelegate>

@property (nonatomic, strong, readonly) UIView *superview;
@property (nonatomic, strong, readonly) UIView *maskView;
@property (nonatomic, strong, readonly) UIView *contentView;
@property (nonatomic, assign, readonly) CGFloat dropAngle;
@property (nonatomic, assign, readwrite) CGPoint markerCenter;
@property (nonatomic, assign, readonly) GTPopupMaskType maskType;

@end

static void *GTPopupControllerParametersKey = &GTPopupControllerParametersKey;
static void *GTPopupControllerNSTimerKey = &GTPopupControllerNSTimerKey;

@implementation GTPopupController
...
- (instancetype)init {
    return [self initWithMaskType:GTPopupMaskTypeBlackTranslucent];
}

- (instancetype)initWithMaskType:(GTPopupMaskType)maskType {
    if (self = [super init]) {
        _isPresenting = NO;
        _maskType = maskType;
        _layoutType = GTPopupLayoutTypeCenter;
        _dismissOnMaskTouched = YES;

        // setter
        self.maskAlpha = 0.5f;
        self.slideStyle = GTPopupSlideStyleFade;
        ...
        // maskView
        if (maskType == GTPopupMaskTypeBlackBlur || maskType == GTPopupMaskTypeWhiteBlur) {
        ...
        switch (maskType) {
            ...
            case GTPopupMaskTypeWhite:
                _maskView.backgroundColor = [UIColor whiteColor];
                break;
            case GTPopupMaskTypeClear:
                _maskView.backgroundColor = [UIColor clearColor];
                break;
            default: // GTPopupMaskTypeBlackTranslucent
...
- (void)presentContentView:(UIView *)contentView
    ...
    [parameters setValue:@(duration) forKey:@"gt_duration"];
    [parameters setValue:@(isSpringAnimated) forKey:@"gt_springAnimated"];
    objc_setAssociatedObject(self, GTPopupControllerParametersKey, parameters, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    ...
    void (^presentCallback)(void) = ^() {
        ...
        weakSelf.popupView.userInteractionEnabled = YES;
        ...
        if (displayTime) {
            ...
            objc_setAssociatedObject(self, GTPopupControllerNSTimerKey, timer, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        }
    };
    ...
    if (isSpringAnimated) {
        [UIView animateWithDuration:duration delay:0.f usingSpringWithDamping:0.6 initialSpringVelocity:0.2 options:UIViewAnimationOptionCurveLinear animations:^{
            ...
            weakSelf.popupView.center = [self finishedCenter];
...
- (void)fadeDismiss {
    ...
    _slideStyle = GTPopupSlideStyleFade;
    [self dismiss];
}

- (void)dismiss {
    id object = objc_getAssociatedObject(self, GTPopupControllerParametersKey);
    if (object && [object isKindOfClass:[NSDictionary class]]) {
        NSTimeInterval duration = 0.0;
        NSNumber *durationNumber = [object valueForKey:@"gt_duration"];
        if (nil != durationNumber) duration = durationNumber.doubleValue;
        BOOL flag = NO;
        NSNumber *flagNumber = [object valueForKey:@"gt_springAnimated"];
...
- (void)dismissWithDuration:(NSTimeInterval)duration springAnimated:(BOOL)isSpringAnimated {
    ...
    void (^dismissCallback)(void) = ^() {
        weakSelf.slideStyle = [objc_getAssociatedObject(self, @selector(fadeDismiss)) integerValue];
        [self removeSubviews];
        weakSelf.isPresenting = NO;
        weakSelf.popupView.transform = CGAffineTransformIdentity;
    ...
    UIViewAnimationOptions (^animOpts)(GTPopupSlideStyle) = ^(GTPopupSlideStyle slide){
        if (slide != GTPopupSlideStyleShrinkInOut1) {
    ...
    if (isSpringAnimated) {
        ...
        [UIView animateWithDuration:duration1 delay:0.f options:UIViewAnimationOptionCurveEaseInOut animations:^{
            ...
            weakSelf.popupView.center = [self bufferCenter:30];
            ...
            [UIView animateWithDuration:duration2 delay:0.f options:animOpts(self.slideStyle) animations:^{
                ...
                weakSelf.popupView.center = [self dismissedCenter];
...
- (void)dropAnimatedWithRotateAngle:(CGFloat)angle {
    _dropAngle = angle;
    _slideStyle = GTPopupSlideStyleFromTop;
}

- (BOOL)dropSupport {
    return (_layoutType == GTPopupLayoutTypeCenter && _slideStyle == GTPopupSlideStyleFromTop);
}

static CGFloat gt_randomValue(int i, int j) {
...
- (void)prepareDropAnimated {
    if (_dropAngle && [self dropSupport]) {
        ...
        transform = CATransform3DRotate(transform,
                                        gt_randomValue(_dropAngle, -_dropAngle) * M_PI / 180,
                                        0, 0, 1.0);
...
- (void)finishedBackground {
    switch (_maskType) {
        case GTPopupMaskTypeBlackTranslucent:
            _maskView.backgroundColor = [UIColor colorWithWhite:0.0 alpha:_maskAlpha];
            break;
        case GTPopupMaskTypeWhite:
            _maskView.backgroundColor = [UIColor whiteColor];
...
- (void)bufferBackground {
    switch (_maskType) {
        case GTPopupMaskTypeBlackBlur:
        case GTPopupMaskTypeWhiteBlur: break;
        case GTPopupMaskTypeBlackTranslucent:
...
- (CGPoint)prepareCenter {
    if (_layoutType == GTPopupLayoutTypeCenter) {
        CGPoint point = _maskView.center;
        if (_slideStyle == GTPopupSlideStyleShrinkInOut1) {
            _popupView.transform = CGAffineTransformMakeScale(0.15, 0.15);
        } else if (_slideStyle == GTPopupSlideStyleShrinkInOut2) {
            _popupView.transform = CGAffineTransformMakeScale(0.8, 0.8);
        } else if (_slideStyle == GTPopupSlideStyleFade) {
            _maskView.alpha = 0;
        } else {
...
- (CGPoint)finishedCenter {
    CGPoint point = _maskView.center;
    switch (_layoutType) {
        case GTPopupLayoutTypeTop:
            return CGPointMake(point.x,
                               _popupView.bounds.size.height / 2);
        case GTPopupLayoutTypeBottom:
            return CGPointMake(point.x,
                               _maskView.bounds.size.height - _popupView.bounds.size.height / 2);
        case GTPopupLayoutTypeLeft:
            return CGPointMake(_popupView.bounds.size.width / 2,
                               point.y);
        case GTPopupLayoutTypeRight:
            return CGPointMake(_maskView.bounds.size.width - _popupView.bounds.size.width / 2,
                               point.y);
        default: // GTPopupLayoutTypeCenter
        {
            if (_slideStyle == GTPopupSlideStyleShrinkInOut1 ||
                _slideStyle == GTPopupSlideStyleShrinkInOut2) {
...
- (CGPoint)dismissedCenter {
    ...
    switch (_slideStyle) {
        ...
        case GTPopupSlideStyleFromBottom:
            return _dismissOppositeDirection ?
            CGPointMake(_popupView.center.x,
                        -_popupView.bounds.size.height / 2) :
            CGPointMake(_popupView.center.x,
        ...
        case GTPopupSlideStyleFromLeft:
            return _dismissOppositeDirection ?
        ...
        case GTPopupSlideStyleFromRight:
            return _dismissOppositeDirection ?
            CGPointMake(-_popupView.bounds.size.width / 2,
                        _popupView.center.y) :
            CGPointMake(_maskView.bounds.size.width + _popupView.bounds.size.width / 2,
...
- (void)destroyTimer {
    id value = objc_getAssociatedObject(self, GTPopupControllerNSTimerKey);
    if (value) {
        [(NSTimer *)value invalidate];
        objc_setAssociatedObject(self, GTPopupControllerNSTimerKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
}
...
- (UIWindow *)frontWindow {
    ...
    if (!applicationWindow) NSLog(@"** GTPopupController ** Window is nil!");
...
- (void)keyboardWillChangeFrame:(NSNotification*)notification {
    ...
    [UIView animateWithDuration:duration delay:0 options:options animations:^{
        if (keyboardHeight > 0) {
            ...
            if (!CGPointEqualToPoint(CGPointZero, weakSelf.markerCenter)) {
                weakSelf.popupView.center = CGPointMake(weakSelf.markerCenter.x, weakSelf.markerCenter.y - changeHeight);
            } else {
                weakSelf.popupView.center = CGPointMake(weakSelf.popupView.center.x, weakSelf.popupView.center.y - changeHeight);
            }
        ...
        weakSelf.markerCenter = [weakSelf finishedCenter];
    }];
}
        ...
        case UIGestureRecognizerStateChanged: {
            switch (_layoutType) {
                case GTPopupLayoutTypeCenter: {
                    ...
                    switch (_slideStyle) {
                        case GTPopupSlideStyleFromLeft:
                        case GTPopupSlideStyleFromRight: break;
                        default:
                        ...
                        case GTPopupSlideStyleFade: {
                            if (g.view.center.y < _maskView.bounds.size.height * 0.25) {
                                ...
                                if (g.view.center.y > _maskView.bounds.size.height * 0.75) {
                                    _slideStyle = GTPopupSlideStyleFromBottom;
                                }
                            }
...
@implementation UIViewController (GTPopupController)

- (GTPopupController *)gt_popupController {
    id popupController = objc_getAssociatedObject(self, _cmd);
    if (nil == popupController) {
        popupController = [[GTPopupController alloc] init];
        self.gt_popupController = popupController;
    }
    return popupController;
}

- (void)setGt_popupController:(GTPopupController *)gt_popupController {
    objc_setAssociatedObject(self, @selector(gt_popupController), gt_popupController, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
