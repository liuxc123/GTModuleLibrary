...
- (BOOL)isJailbroken {
    ...
    NSString *path = [NSString stringWithFormat:@"/private/%@", [NSString gt_stringWithUUID]];
...
static uint64_t yy_net_counter_get_by_type(yy_net_interface_counter *counter, GTNetworkTrafficType type) {
    uint64_t bytes = 0;
    if (type & GTNetworkTrafficTypeWWANSent) bytes += counter->pdp_ip_out;
    if (type & GTNetworkTrafficTypeWWANReceived) bytes += counter->pdp_ip_in;
    if (type & GTNetworkTrafficTypeWIFISent) bytes += counter->en_out;
    if (type & GTNetworkTrafficTypeWIFIReceived) bytes += counter->en_in;
    if (type & GTNetworkTrafficTypeAWDLSent) bytes += counter->awdl_out;
    if (type & GTNetworkTrafficTypeAWDLReceived) bytes += counter->awdl_in;
    return bytes;
}
