...
@implementation UIImage (GTBetterFace)

- (UIImage *)gt_betterFaceImageForSize:(CGSize)size
                           accuracy:(GTAccuracy)accurary;
{
    NSArray *features = [UIImage _gt_faceFeaturesInImage:self accuracy:accurary];
    ...
    if ([features count]==0) {
        ...
        return [self _gt_subImageForFaceFeatures:features
                                         size:size];
    }
}

- (UIImage *)_gt_subImageForFaceFeatures:(NSArray *)faceFeatures size:(CGSize)size
{
...
+ (NSArray *)_gt_faceFeaturesInImage:(UIImage *)image accuracy:(GTAccuracy)accurary
{
    CIImage *ciImage = [CIImage imageWithCGImage:image.CGImage];
    NSString *accuraryStr = (accurary == GTAccuracyLow) ? CIDetectorAccuracyLow : CIDetectorAccuracyHigh;
