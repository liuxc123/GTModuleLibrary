...
@implementation NSObject (GTReflection)
...
{
    return [[self class] gt_propertyKeys];
}
+ (NSArray *)gt_propertyKeys {
...
{
    return [[self class] gt_propertiesInfo];
}
/**
...
+ (NSArray *)gt_propertiesInfo
{
    ...
    for (int i = 0; i < propertyCount; i++)
    {
        [propertieArray addObject:({
            ...
            NSDictionary *dictionary = [self gt_dictionaryWithProperty:properties[i]];
...
+ (NSArray *)gt_propertiesWithCodeFormat
{
    ...
    NSArray *properties = [[self class] gt_propertiesInfo];
...
-(NSArray*)gt_methodListInfo{
    ...
    for (int i = 0; i < count ; i++)
    {
        ...
        for (int index=0; index<argumentsCount; index++) {
            ...
            [arguments addObject:[[self class] gt_decodeType:arg]];
        }

        NSString *returnTypeString =[[self class] gt_decodeType:returnType];
        NSString *encodeString = [[self class] gt_decodeType:encoding];
...
-(NSDictionary *)gt_protocolList{
    return [[self class]gt_protocolList];
}
+ (NSDictionary *)gt_protocolList
{
...
+ (NSArray *)gt_instanceVariable
{
    ...
    NSMutableArray *result = [NSMutableArray array];
    for (int i = 0; i < outCount; i++) {
        NSString *type = [[self class] gt_decodeType:ivar_getTypeEncoding(ivars[i])];
...
#pragma mark -- help
+ (NSDictionary *)gt_dictionaryWithProperty:(objc_property_t)property
{
...
+ (NSString *)gt_decodeType:(const char *)cString
{
    ...
    } else
    {
        if ([[result substringToIndex:1] isEqualToString:@"^"]) {
            result = [NSString stringWithFormat:@"%@ *",
                      [NSString gt_decodeType:[[result substringFromIndex:1] cStringUsingEncoding:NSUTF8StringEncoding]]];
        }
    }
    return result;
}
