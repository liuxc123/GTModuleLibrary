...
#import "NSDictionary+GTKit.h"
#import "NSString+GTKit.h"
#import "NSData+GTKit.h"

@interface _GTXMLDictionaryParser : NSObject <NSXMLParserDelegate>
@end

@implementation _GTXMLDictionaryParser {
...
- (void)textEnd {
    _text = _text.gt_stringByTrim.mutableCopy;
    if (_text.length) {
...
@implementation NSDictionary (GTKit)

+ (NSDictionary *)gt_dictionaryWithPlistData:(NSData *)plist {
    if (!plist) return nil;
...
+ (NSDictionary *)gt_dictionaryWithPlistString:(NSString *)plist {
    if (!plist) return nil;
    NSData *data = [plist dataUsingEncoding:NSUTF8StringEncoding];
    return [self gt_dictionaryWithPlistData:data];
}
...
- (NSString *)gt_plistString {
    ...
    if (xmlData) return xmlData.gt_utf8String;
    return nil;
}
...
- (NSArray *)gt_allValuesSortedByKeys {
    NSArray *sortedKeys = [self gt_allKeysSorted];
...
- (NSString *)gt_jsonPrettyStringEncoded {
    if ([NSJSONSerialization isValidJSONObject:self]) {
        NSError *error;
...
+ (NSDictionary *)gt_dictionaryWithXML:(id)xml {
    _GTXMLDictionaryParser *parser = nil;
    if ([xml isKindOfClass:[NSString class]]) {
        parser = [[_GTXMLDictionaryParser alloc] initWithString:xml];
    } else if ([xml isKindOfClass:[NSData class]]) {
        parser = [[_GTXMLDictionaryParser alloc] initWithData:xml];
    }
...
- (NSString *)gt_URLQueryString
{
    ...
    for (NSString *key in [self allKeys]) {
        ...
        NSString *escaped = [[[self objectForKey:key] description] gt_urlEncode];
...
- (NSNumber *)gt_numberValueForKey:(NSString *)key default:(NSNumber *)def {
    if (!key) return def;
    id value = self[key];
    if (!value || value == [NSNull null]) return def;
    if ([value isKindOfClass:[NSNumber class]]) return value;
...
- (NSString *)gt_stringValueForKey:(NSString *)key default:(NSString *)def {
    if (!key) return def;
    id value = self[key];
    if (!value || value == [NSNull null]) return def;
...
@implementation NSMutableDictionary (GTKit)

+ (NSMutableDictionary *)gt_dictionaryWithPlistData:(NSData *)plist {
    if (!plist) return nil;
...
+ (NSMutableDictionary *)gt_dictionaryWithPlistString:(NSString *)plist {
    if (!plist) return nil;
    NSData *data = [plist dataUsingEncoding:NSUTF8StringEncoding];
    return [self gt_dictionaryWithPlistData:data];
}

- (id)gt_popObjectForKey:(id)aKey {
    if (!aKey) return nil;
    id value = self[aKey];
