...
+ (NSString *)gt_generateURLWithPattern:(NSString *)pattern parameters:(NSArray *)parameters {
    return [[self class] gt_generateURLWithPattern:pattern parameters:parameters extraParameters:nil];
}
...
+ (NSString *)gt_generateURLWithPattern:(NSString *)pattern parameters:(NSArray *)parameters extraParameters:(NSDictionary *)extraParameters {
    // 创建路由
    JLRRouteDefinition *route = [[JLRRouteDefinition alloc] initWithPattern:pattern priority:0 handlerBlock:nil];

    NSArray *patternComponents;
    @try {
        patternComponents = [route mutableArrayValueForKeyPath:@"patternPathComponents"];
    ...
    if (routeValues && routeValues.count) {
        ...
        return [NSString stringWithFormat:@"/%@%@", urlPath, [self gt_mapDictionaryToURLQueryString:extraParameters]];
    }
    return nil;
}
...
+ (BOOL)routeURL:(NSURL *)URL withParameters:(NSMutableDictionary<NSString *,id> *)parameters callBack:(void (^)(id))callBack {
    ...
    if (callBack) {
        [params setObject:callBack forKey:JLRouterParameterCallBack];
    }
    return [self routeURL:URL withParameters:params];
};
